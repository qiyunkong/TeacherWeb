  ...
  (function () {
    ...
    function Sphere() {
      if (Sphere.arr) {
        ...
        var arr = [1, 3, 7, 9, 11, 14, 21, 20, 12, 10, 9, 7, 1], // 每一层li的个数
          arrLen = arr.length, // 获取又多少层
          xDeg = 180 / (arrLen - 1);  // 每一层旋绕X轴转多少度

        for (var i = 0; i < len; i++) {
          // 获取每一个li 
          // 定义变量来计算当前的li是第几层,已经在当前层是第几个,
          var numC = 0; // 计算当前li是处于第几层
          var numG = 0; // 用来保存当前li是当前成的第几个
          var arrSum = 0; // 用来就散当前成是多好个  
          ...
          for (var j = 0; j < arrLen; j++) {
            ...
            if (arrSum > i) {
              numC = j;    // 计算li在几层
              numG = arr[j] - (arrSum - i);   // 计算当前的li是当前层的第几个
              break;
            }
          }

          var yDeg = 360 / arr[numC];  // 计算没一层内部的每个里绕Y轴旋转读书
    ...
    function Helix() {
      if (Helix.arr) {
        ...
        var h = 4,  // 圈数
          tY = 7,  // 每个li上下错位值
          num = Math.round(len / h),  // 每一圈多少个+
          deg = 360 / num,  // 每一个li旋转的角度
          mid = len / 2 - 0.5;  //  找准ul的中间值
